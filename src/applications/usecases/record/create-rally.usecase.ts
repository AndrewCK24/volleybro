  ...
  async execute(
    ...
    if (rally.win) {
      if (homePlayerIndex !== -1) {
        homePlayer.stats[setIndex][rally.home.type].success += 1;
        record.teams.home.players[homePlayerIndex] = homePlayer;
      }
      homeTeam.stats[setIndex][rally.home.type].success += 1;
      awayTeam.stats[setIndex][rally.away.type].error += 1;
    } else {
      if (homePlayerIndex !== -1) {
        homePlayer.stats[setIndex][rally.home.type].error += 1;
        record.teams.home.players[homePlayerIndex] = homePlayer;
      }
      homeTeam.stats[setIndex][rally.home.type].error += 1;
      awayTeam.stats[setIndex][rally.away.type].success += 1;
    }
    ...
    await this.recordRepository.update({ _id: recordId }, record);
