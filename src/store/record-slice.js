		...
		setRecordingDetail: (state, action) => {
			...
			if (state.status.recordingNum === 0) {
				...
				state.data = {
					...
					typeNum: action.payload.typeNum,
					win: action.payload.win,
					scoreOurs: state.setData.records[state.status.recordingNum - 1].scoreOurs + oursPt,
					scoreOppo: state.setData.records[state.status.recordingNum - 1].scoreOppo + oppoPt,
				};
			}
		},
		createRecord: (state) => {
