...
const jwtSignIn = require("./jwt-sign-in.cjs");
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log(
      `[AUTH] token verified for USER ${cookieUser.email} (${cookieUser._id})`
    );

    await connectMongoDB.handler();
    const dbUser = await User.findById(cookieUser._id);
    console.log(
      `[AUTH] dbUser found for USER ${cookieUser.email} (${cookieUser._id})`
    );
    if (cookieUser.password === dbUser.password) {
      console.log(
        `[AUTH] passwords match for USER ${cookieUser.email} (${cookieUser._id})`
      );
      const newToken = jwtSignIn.handler(dbUser);
      ...
      console.log(
        `[AUTH] passwords do not match for USER ${cookieUser.email} (${cookieUser._id})`
      );
      return {
        status: 400,
